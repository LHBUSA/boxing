...
Branch `boxing-core-v1`. **No production deployment.** Migrations 0001–0012 are applied to the dedicated Boxing STAGING Supabase project only (`propbetedge-boxing-staging`, ref `wpaxofilvbsjyrxrwjhg`, created 2026-09-13; no production data, never the UFC/NFL/MLB projects). Since 2026-09-13 the staging-only Worker `boxing-odds-staging` (no routes, cron) captures The Odds API boxing prices into staging.

The whole suite runs on disposable PostgreSQL 17, locally and in CI (`postgres:17`): 156 unit/worker tests and 148 database tests. The test harness still refuses non-local hosts; staging is checked by `scripts/staging/verify-staging.ps1` (32 SQL + HTTP checks, all writes rolled back).
...
| Odds capture, derivations, freshness, `MARKET_MOVED` | #4 | **Forward capture live in STAGING** (rights approved 2026-09-13, raw redistribution prohibited); provider ledger preserves unmatched events; canonical ticks need canonical bouts |
...
3. ~~Review and enable `the_odds_api`, deploy `boxing-odds` against staging~~ done (staging). Production capture needs a production project added to the write-target allow-list (reviewed code change).
