...
//
// ONE switch: `LAUNCH_APPROVED` in web/lib/posture.ts, committed by owner decision.
//
// BUILD MODE (LAUNCH_APPROVED = false) is how the product is built: main deploys natively on Vercel and is
// visible at boxing.propbetedge.ai. Attaching that domain is NOT a failure. What is refused:
//   - pages without <meta name="robots" content="noindex, nofollow">, or responses without X-Robots-Tag
//   - robots.txt that does not block every crawler, a sitemap, or an IndexNow integration
//   - canonical / og:url, or public-domain URLs in pages or metadata (no launch URLs)
//   - an indexing switch anywhere but lib/posture.ts (the retired BOXING_ALLOW_INDEXING / BOXING_PUBLIC_URL)
// LAUNCH MODE (LAUNCH_APPROVED = true) expects the opposite discovery posture: index/follow, robots allowing
// crawlers, canonical/og:url on PUBLIC_URL.
// In BOTH modes:
//   - any domain other than *.vercel.app and boxing.propbetedge.ai attached to project `boxing`
...
//   node scripts/posture.mjs live [baseUrl]    probe a running deployment anonymously (default boxing.propbetedge.ai)
//   node scripts/posture.mjs vercel            project domains + env key names via the Vercel API
//
// Every mode exits 1 on any failure. `npm run build` runs static + bundle, so a Vercel build that
// breaks posture fails and is never promoted.
//
// Authority: attached domains come only from the Vercel project domain list (`vercel` mode). The build-time
// system variable VERCEL_PROJECT_PRODUCTION_URL is not evidence of anything and is not read.
...
// The committed launch switch, read from web/lib/posture.ts (the only place it may live).
export function readLaunch(web = WEB) {
  const src = readFileSync(join(web, "lib", "posture.ts"), "utf8");
  const flag = src.match(/^export const LAUNCH_APPROVED(?::\s*boolean)?\s*=\s*(true|false);$/m);
  const url = src.match(/^export const PUBLIC_URL = "([^"]+)";$/m);
  return { declared: Boolean(flag && url), launchApproved: flag?.[1] === "true", publicUrl: url?.[1] ?? null, src };
}
export const LAUNCH_APPROVED = readLaunch().launchApproved;

const failures = [];
const passes = [];
const check = (ok, label, detail = "") => { (ok ? passes : failures).push(detail ? `${label}: ${detail}` : label); return ok; };
const modeName = (launch) => (launch ? "launch mode" : "build mode");
...
function staticChecks({ launchApproved } = {}) {
  ...
  const launchOn = launchApproved ?? launch.launchApproved;
  check(launch.declared, "lib/posture.ts declares LAUNCH_APPROVED and PUBLIC_URL", `LAUNCH_APPROVED=${launch.launchApproved} (${modeName(launchOn)})`);
  check(launch.publicUrl === LIVE_BASE, `PUBLIC_URL is ${LIVE_BASE}`, launch.publicUrl ?? "missing");
  check(/^export const INDEXABLE = LAUNCH_APPROVED && process\.env\.VERCEL_ENV === "production";$/m.test(launch.src), "indexing requires LAUNCH_APPROVED + a production deployment");
  check(/^export const BASE_URL = INDEXABLE\s*\?\s*PUBLIC_URL\s*:/m.test(launch.src) && (launch.src.replace(/\/\/.*$/gm, "").match(/PUBLIC_URL/g) ?? []).length === 2, "PUBLIC_URL is used only for launch-mode metadata");
  check(/^export const canonical = [^\n]*INDEXABLE \? \{ canonical: path \} : undefined/m.test(launch.src), "canonical() emits nothing unless indexable");
  const nextConfig = src("next.config.ts");
  const envSwitch = [...code.map((p) => [rel(p), src(rel(p))]), ["next.config.ts", nextConfig]].filter(([, s]) => /BOXING_ALLOW_INDEXING|BOXING_PUBLIC_URL|VERCEL_PROJECT_PRODUCTION_URL/.test(s)).map(([p]) => p);
  check(!envSwitch.length, "no env-based indexing switch or production-URL guess in web code", envSwitch.join(", "));
  ...
  if (!launchOn) {
    ...
    check(!customRefs.length, `build mode: no source references ${CUSTOM_DOMAIN} outside lib/posture.ts`, customRefs.join(", "));
  }
  ...
  const clientGateway = code.filter((p) => /^["']use client["']/m.test(src(rel(p))) && /@\/lib\/gateway|@\/lib\/posture|from "\.\.?\/.*(gateway|posture)"/.test(src(rel(p)))).map(rel);
  check(!clientGateway.length, "no client component imports the gateway or posture", clientGateway.join(", "));
...
// ---------------------------------------------------------------- bundle
function bundleChecks({ web = WEB, env = process.env, launchApproved } = {}) {
  const launch = launchApproved ?? LAUNCH_APPROVED;
  const next = join(web, ".next");
  if (!check(existsSync(join(next, "static")), `.next/static exists (run after next build; ${modeName(launch)})`)) return;
  ...
  const robotsMeta = (f) => [...readFileSync(f, "utf8").matchAll(/<meta name="robots" content="([^"]+)"/g)].map((m) => m[1]);
  const urlsIn = (f) => [...readFileSync(f, "utf8").matchAll(/<(?:link rel="canonical" href|meta property="og:url" content)="([^"]+)"/g)].map((m) => m[1]);
  ...
  if (!launch) {
    ...
    check(!custom.length, `build mode: no ${CUSTOM_DOMAIN} launch URLs in built output`, custom.slice(0, 10).join(", "));
    const unindexed = html.filter((f) => { const m = robotsMeta(f); return !m.length || !m.every((c) => /noindex/.test(c)); }).map((f) => relative(web, f));
    check(html.length > 0 && !unindexed.length, `build mode: ${html.length} prerendered pages carry meta robots noindex`, unindexed.slice(0, 10).join(", "));
    const urls = html.flatMap((f) => urlsIn(f).map((u) => `${u} (${relative(web, f)})`));
    check(!urls.length, "build mode: no canonical/og:url in prerendered pages", urls.slice(0, 10).join(", "));
    check(/User-Agent: \*\nDisallow: \/\n?/i.test(robots) && !/^Allow:/im.test(robots) && !/Sitemap:/i.test(robots), "build mode: built robots.txt blocks all crawlers, no sitemap", JSON.stringify(robots.slice(0, 80)));
  } else {
    const noindexed = html.filter((f) => robotsMeta(f).some((c) => /noindex/.test(c)) && !/_not-found/.test(f)).map((f) => relative(web, f));
    check(!noindexed.length, "launch mode: prerendered pages are indexable", noindexed.slice(0, 10).join(", "));
    const foreign = html.flatMap((f) => urlsIn(f).filter((u) => !u.startsWith(LIVE_BASE)).map((u) => `${u} (${relative(web, f)})`));
    check(!foreign.length, `launch mode: canonical/og:url only on ${LIVE_BASE}`, foreign.slice(0, 10).join(", "));
    check(!/Disallow: \/\s*$/im.test(robots), "launch mode: built robots.txt allows crawlers", JSON.stringify(robots.slice(0, 80)));
  }
  ...
  const legacy = ["BOXING_ALLOW_INDEXING", "BOXING_PUBLIC_URL"].filter((k) => env[k]);
  check(!legacy.length, "no env indexing switch in the build environment (the switch is committed in lib/posture.ts)", legacy.join(", "));
}

// ---------------------------------------------------------------- live
async function liveChecks(base = LIVE_BASE, { launchApproved } = {}) {
  const launch = launchApproved ?? LAUNCH_APPROVED;
  base = base.replace(/\/$/, "");
  const values = secretValues();
  ...
  if (!launch) {
    check(robots.res.status === 200 && /User-Agent: \*\nDisallow: \/\n?/i.test(rb) && !/^Allow:/im.test(rb) && !/Sitemap:/i.test(rb), "build mode: robots.txt blocks all crawlers, no sitemap", JSON.stringify(rb.slice(0, 120)));
    const sitemap = await get("/sitemap.xml");
    check(sitemap.res.status === 404, "build mode: sitemap.xml absent", `status ${sitemap.res.status}`);
  } else {
    check(robots.res.status === 200 && !/Disallow: \/\s*$/im.test(rb), "launch mode: robots.txt allows crawlers", JSON.stringify(rb.slice(0, 120)));
  }
  ...
  const pages = ["/", "/fight-week", "/events", "/fighters", "/titles", "/rankings", "/odds", "/news", "/videos", "/scorecards", "/judges", "/referees", "/officials", "/promoters", "/history", "/hall-of-fame", "/methodology", "/this-page-does-not-exist",
    pick("/fights/"), pick("/events/"), pick("/fighters/"), pick("/scorecards/"), pick("/officials/"), pick("/promoters/")].filter(Boolean);
  ...
  for (const path of pages) {
    ...
    const og = body.match(/<meta property="og:url" content="([^"]+)"/)?.[1];
    if (!launch) {
      ...
      check(/noindex/.test(header) && /nofollow/.test(header), `${path} X-Robots-Tag noindex, nofollow`, header || "missing");
      check(!canon && !og, `${path} no canonical/og:url (build mode)`, [canon, og].filter(Boolean).join(" "));
      check(!body.includes(CUSTOM_DOMAIN), `${path} no ${CUSTOM_DOMAIN} launch URLs in the page`, (body.match(new RegExp(`.{60}${CUSTOM_DOMAIN.replace(/\./g, "\\.")}.{20}`)) ?? [""])[0]);
    } else if (res.status === 200) {
      check(!robotsMeta.some((c) => /noindex/.test(c)) && !/noindex/.test(header), `${path} indexable (launch mode)`, `${robotsMeta.join(" | ")} ${header}`);
      check((!canon || canon.startsWith(LIVE_BASE)) && (!og || og.startsWith(LIVE_BASE)), `${path} canonical/og:url on ${LIVE_BASE}`, [canon, og].filter(Boolean).join(" "));
    }
  ...
  const assetHits = [];
  for (const a of assets) { const { body } = await get(a); assetHits.push(...scanText(body, a, values)); if (!launch && body.includes(CUSTOM_DOMAIN)) assetHits.push(`${CUSTOM_DOMAIN} in ${a}`); }
  ...
  for (const path of writeTargets) for (const method of ["POST", "PUT", "PATCH", "DELETE"]) {
    ...
    const readOnlyRender = /^\/(fights|events|fighters)\//.test(path) && res.status === 200 && /text\/html/.test(res.headers.get("content-type") ?? "") && /<html/.test(body);
...
// ---------------------------------------------------------------- vercel
// Pure decision over the project's CURRENT domain list. boxing.propbetedge.ai is the build surface and may be
// attached in either mode; anything else outside *.vercel.app is unexpected.
export function domainPosture(names) {
  const unexpected = names.filter((n) => { const d = String(n).toLowerCase(); return !d.endsWith(".vercel.app") && d !== CUSTOM_DOMAIN; });
  return { unexpected, customAttached: names.some((n) => String(n).toLowerCase() === CUSTOM_DOMAIN), ok: unexpected.length === 0 };
}
...
function vercelApi() {
  ...
  return async (path) => {
    ...
    if (!res.ok) throw new Error(`${path} -> ${res.status}${res.status === 403 ? " (token invalid or expired: run `vercel login` or set VERCEL_TOKEN)" : ""}`);
    return res.json();
  };
}

async function vercelChecks({ api = vercelApi() } = {}) {
  ...
  try {
    ...
    // Fail closed when the list is empty or truncated: an unread page could hold an unexpected domain.
    ...
    const domains = domainPosture(names);
    check(domains.ok, `attached domains are *.vercel.app or ${CUSTOM_DOMAIN}`, `domains: ${names.join(", ")}`);
    ...
    check(!indexing.length, "no env indexing switch on Vercel (the switch is committed in lib/posture.ts)", indexing.join(", "));
  } catch (err) {
    check(false, "Vercel API reachable with this token", String(err.message));
  }
}

// ---------------------------------------------------------------- main
// opts (tests): { web, env, launchApproved } for bundle, { launchApproved } for static/live, { api } for vercel.
export async function runPosture(modes, base, opts = {}) {
  failures.length = 0; passes.length = 0;
  for (const mode of modes) {
    if (mode === "static") staticChecks(opts);
    else if (mode === "bundle") bundleChecks(opts);
    else if (mode === "live") await liveChecks(base ?? LIVE_BASE, opts);
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  ...
  if (!["static", "bundle", "live", "vercel", "all"].includes(mode)) {
    console.error("usage: node scripts/posture.mjs static | bundle | vercel | live [baseUrl] | all [baseUrl]");
    process.exit(2);
  }
  console.log(`posture: LAUNCH_APPROVED=${LAUNCH_APPROVED} (${modeName(LAUNCH_APPROVED)})`);
