...
export function pgStore(client) {
  ...
  return {
    ...
    async titleChainPairs(orgSlug, limit) { return (await one(client, 'select public.boxing_title_chain_pairs($1, $2) as r', [orgSlug, limit])).r; },
    async recordTitleAnalyses(p) { return (await one(client, 'select public.boxing_record_title_analyses($1) as r', [p])).r; },
    async titleProposalSummary() { return (await one(client, 'select public.boxing_title_proposal_summary() as r')).r; },
    ...
    async siteFighterContext(ref) { return (await one(client, 'select public.boxing_site_fighter_context($1) as r', [ref])).r; },
    async siteBoutContext(ref) { return (await one(client, 'select public.boxing_site_bout_context($1) as r', [ref])).r; },
    async siteHallOfFame(category, year, limit, offset) { return (await one(client, 'select public.boxing_site_hall_of_fame($1, $2, $3, $4) as r', [category, year, limit, offset])).r; },
    async siteHistory() { return (await one(client, 'select public.boxing_site_history() as r')).r; },
    async siteWire(limit) { return (await one(client, 'select public.boxing_site_wire($1) as r', [limit])).r; },
