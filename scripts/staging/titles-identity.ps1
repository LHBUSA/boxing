# Sanctioning-body identity review candidates on Boxing STAGING: fill the printed-name map, refresh, print the summary.
#
...
try {
  ...
  $nodeArgs = @((Join-Path $root 'scripts/titles/identity-candidates.mjs'), "--batch=$Batch")
  node @nodeArgs
  if ($LASTEXITCODE -ne 0) { throw "identity-candidates exited $LASTEXITCODE" }
} finally {
